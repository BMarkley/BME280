 ...
 * @version 1.2
 * 
 * @changes Edited the Function declarations so that they work alongside Simpletools "readbyte" and "writebyte"
 * 
*/
...
void BME280_writeByte(unsigned char , unsigned char , unsigned char );
unsigned char BME280_readByte(unsigned char , unsigned char);
void BME280_readBytes(unsigned char , unsigned char , unsigned char , unsigned char *);
...
int BME280_getID()
{
  int i = BME280_readByte(BME280_ADDRESS, BME280_ID);
  return i;
}

void BME280_reset()
{
  BME280_writeByte(BME280_ADDRESS, BME280_RS, 0xB6);
  pause(300);
}
...
int BME280_getStatus()
{
  ...
  i = BME280_readByte(BME280_ADDRESS, BME280_STATUS);
  return i;
}
...
void BME280_setTemp(int t)
{
  ...
  i = BME280_readByte(BME280_ADDRESS, BME280_CNTRL);
  t = t << 3;
  i = i & 0xe3;
  i = i | t;
  BME280_writeByte(BME280_ADDRESS, BME280_CNTRL, i);
}
...
void BME280_setHumidity(int h)
{
  ...
  BME280_writeByte(BME280_ADDRESS, BME280_CH, h);
}
...
void BME280_setStandbyRate(int s)
{
  ...
  i = BME280_readByte(BME280_ADDRESS, BME280_CNFG);
  s = s << 5;
  i = i & 0x1f;
  i = i | s;
  BME280_writeByte(BME280_ADDRESS, BME280_CNFG, i);
}
...
int BME280_getPressure(void)
{
  ...
  BME280_readBytes(BME280_ADDRESS, BME280_PD, 3, _Buffer);
...
int BME280_getTemp(void)
{
  ...
  BME280_readBytes(BME280_ADDRESS, BME280_TD, 3, _Buffer);
...
int BME280_getHumidity(void)
{
  ...
  BME280_readBytes(BME280_ADDRESS, BME280_HD, 2, _Buffer);
...
void BME280_calibration(void)
{
  ...
  BME280_readBytes(BME280_ADDRESS, BME280_T1, 6, _Buffer);
  j = _Buffer[1];
  j = j << 8;
  j = j | _Buffer[0];
  _cal.T1 = j;
...
//  print("T1: %d, T2: %d, T3: %d\n", _cal.T1, _cal.T2, _cal.T3);
  ...
  BME280_readBytes(BME280_ADDRESS, BME280_P1, 18, _Buffer);
  j = _Buffer[1];
  j = j << 8;
  j = j | _Buffer[0];
  _cal.P1 = j;
...
//    _cal.P1, _cal.P2, _cal.P3, _cal.P4, _cal.P5, _cal.P6, _cal.P7, _cal.P8, _cal.P9);
  ...
  j = BME280_readByte(BME280_ADDRESS, BME280_H1);
  _cal.H1 = j;
  
  BME280_readBytes(BME280_ADDRESS, BME280_H2, 7, _Buffer);
  i = _Buffer[1];
  i = i << 8;
  i = i | _Buffer[0];
  _cal.H2 = i;
 ...
 * @param data to write
*/
void BME280_writeByte(unsigned char address, unsigned char subAddress, unsigned char data)
{
 ...
 * @return byte value
*/
uint8_t BME280_readByte(unsigned char address, unsigned char subAddress)
{
  uint8_t data;
...
*/
void BME280_readBytes(unsigned char address, unsigned char subAddress, unsigned char cnt, unsigned char *dest)
{
  i2c_in(_bme, address, subAddress, 1, dest, cnt);
}

/**
 * @detail define double to enable floating point calcs
*/
#ifdef DODOUBLE

float BME280_getTemperaturef()
{
  int t;
  double v1, v2;
