...
int BME280_getID()
{
  int i = readByte(BME280_ADDRESS, BME280_ID);
  return i;
}

void BME280_reset()
{
  writeByte(BME280_ADDRESS, BME280_RS, 0xB6);
  pause(300);
}
...
int BME280_getStatus()
{
  ...
  i = readByte(BME280_ADDRESS, BME280_STATUS);
  return i;
}
...
void BME280_setTemp(int t)
{
  ...
  i = readByte(BME280_ADDRESS, BME280_CNTRL);
  t = t << 3;
  i = i & 0xe3;
  i = i | t;
  writeByte(BME280_ADDRESS, BME280_CNTRL, i);
}
...
void BME280_setHumidity(int h)
{
  ...
  writeByte(BME280_ADDRESS, BME280_CH, h);
}
...
void BME280_setStandbyRate(int s)
{
  ...
  i = readByte(BME280_ADDRESS, BME280_CNFG);
  s = s << 5;
  i = i & 0x1f;
  i = i | s;
  writeByte(BME280_ADDRESS, BME280_CNFG, i);
}
...
int BME280_getPressure(void)
{
  ...
  readBytes(BME280_ADDRESS, BME280_PD, 3, _Buffer);
...
int BME280_getTemp(void)
{
  ...
  readBytes(BME280_ADDRESS, BME280_TD, 3, _Buffer);
...
int BME280_getHumidity(void)
{
  ...
  readBytes(BME280_ADDRESS, BME280_HD, 2, _Buffer);
...
void BME280_calibration(void)
{
  ...
  readBytes(BME280_ADDRESS, BME280_T1, 6, _Buffer);
  j = _Buffer[1];
  j = j << 8;
  j = j | _Buffer[0];
  _cal.T1 = j;
...
//  print("T1: %d, T2: %d, T3: %d\n", _cal.T1, _cal.T2, _cal.T3);
  ...
  readBytes(BME280_ADDRESS, BME280_P1, 18, _Buffer);
  j = _Buffer[1];
  j = j << 8;
  j = j | _Buffer[0];
  _cal.P1 = j;
...
//    _cal.P1, _cal.P2, _cal.P3, _cal.P4, _cal.P5, _cal.P6, _cal.P7, _cal.P8, _cal.P9);
  ...
  j = readByte(BME280_ADDRESS, BME280_H1);
  _cal.H1 = j;
  
  readBytes(BME280_ADDRESS, BME280_H2, 7, _Buffer);
  i = _Buffer[1];
  i = i << 8;
  i = i | _Buffer[0];
  _cal.H2 = i;
 ...
 * @return byte value
*/
uint8_t readByte(unsigned char address, unsigned char subAddress)
{
  uint8_t data;
